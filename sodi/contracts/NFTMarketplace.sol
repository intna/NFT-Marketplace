...
contract NFTMarketplace is ERC721URIStorage {
  ...
  constructor() ERC721("NFT Metavarse token", "MY NFT") {
  ...
  function reSellToken(uint256 tokenId, uint256 price) public payable {
    require(idMarketItem[tokenId].owner == msg.sender, "? what are you doing");
  ...
  function createMarketSale(uint256 tokenId) public payable {
    require(idMarketItem[tokenId].tokenId != 0, "cant find token");
  ...
  function fetchMyNFT() public view returns (MarketItem[] memory) {
    ...
    uint256 currentIndex = 0;
    MarketItem[] memory tempItems = new MarketItem[](totalCount);

    for (uint256 i = 0; i < totalCount; i++) {
      if (idMarketItem[i + 1].owner == msg.sender) {
        tempItems[currentIndex] = idMarketItem[i + 1];
        currentIndex++;
      }
    }
